...
function makeEvent(filePath, type, offset = 0) {
    return {
        filePath,
        type,
...
describe('ChangeStore in-memory tests', () => {
    test('1. addChange + getRecent returns entries', () => {
        const store = new store_1.ChangeStore(10);
    ...
    test('3. maxSize of 5 keeps exactly the 5 most recent after adding 10 changes', () => {
        const store = new store_1.ChangeStore(5);
        for (let i = 0; i < 10; i++) {
    ...
    test('4. maxSize of 0 does not crash and stores no entries', () => {
        const store = new store_1.ChangeStore(0);
    ...
    test('5. Negative maxSize throws in the constructor', () => {
        expect(() => new store_1.ChangeStore(-1)).toThrow(/maxSize/i);
    });
    ...
    test('8. 100 concurrent Promise.all addChange calls all land', async () => {
        ...
        await Promise.all(Array.from({ length: 100 }, (_, i) => Promise.resolve().then(() => store.addChange(makeEvent(`/tmp/${i}.ts`, 'change', i)))));
        expect(store.getSize()).toBe(100);
    });
});
//# sourceMappingURL=change-store.test.js.map